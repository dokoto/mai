...
export const showDoctors = ({ body, params }, res, next) => {
  const query =
    body && body.id ? { funcRole: consts.DOCTOR, id: body.id } : { funcRole: consts.DOCTOR }
...
export const create = ({ body }, res, next) => User.create(body)
  .then((user) => {
    sign(user.id)
...
export const update = ({ body }, params, user, res, next) => User.findById(params.id === 'me' ? user.id : params.id)
  .then(notFound(res))
  .then((result) => {
...
export const updatePassword = ({ body }, params, user, res, next) => User.findById(params.id === 'me' ? user.id : params.id)
  .then(notFound(res))
  .then((result) => {
    if (!result) return null
    const isSelfUpdate = user.id === result.id
