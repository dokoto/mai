...
test('POST /treatments 201 (master)', async () => {
  const { status, body } = await request(app())
    .post(`${apiRoot}`)
    .send({ access_token: masterKey, name: 'test', text: 'test' })
...
test('POST /treatments 401 (admin)', async () => {
...
test('PUT /treatments/:id 200 (master)', async () => {
  const { status, body } = await request(app())
...
test('PUT /treatments/:id 401 (admin)', async () => {
  const { status } = await request(app())
    .put(`${apiRoot}/${treatment.id}`)
    .send({ access_token: adminSession })
  expect(status).toBe(401)
...
test('PUT /treatments/:id 404 (master)', async () => {
  const { status } = await request(app())
...
test('DELETE /treatments/:id 204 (master)', async () => {
  const { status } = await request(app())
...
test('DELETE /treatments/:id 401 (admin)', async () => {
  const { status } = await request(app())
    .delete(`${apiRoot}/${treatment.id}`)
...
test('DELETE /treatments/:id 404 (master)', async () => {
  const { status } = await request(app())
