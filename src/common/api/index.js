...
export function nextsAppointments() {
  return Get.fetch(paths.nextsAppointments, {
...
export function getToDaySessions(selectedDay) {
  return fetch(`${paths.sessionsBy}/${selectedDay}`).then(response => response.json());
}
...
export function getBusyDays(userId, initDate, numOfMonths) {
  return fetch(`${paths.busyDays}/${userId}/${initDate}/${numOfMonths}`).then(response =>
    response.json(),
  );
}
