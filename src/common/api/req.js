...
class Post extends ReqBase {
   ...
   * @param {Object} [options = {}] Fetch options (body, headers, tokens)
   ...
   * @param {Object} [options = {}] Payload data
  ...
  static response(response) {
    ...
    } else if ([NO_AUTH, RESOURCE_EXIST, BAD_REQUEST].includes(response.status)) {
...
class Put extends ReqBase {
   ...
   * @returns {module:Common/Utils/Api/Req~jsend} jsend object type
   * @example
   * Put.fetch(paths.login, token, options);
   */
  static fetch(path, options = {}) {
    return fetch(path, this.options(options))
      .then(this.response)
   ...
   */
  static response(response) {
    const { SUCCESS, NO_CONTENT, NOT_FOUND, NO_AUTH, BAD_REQUEST, ERROR } = CODES.PUT;
